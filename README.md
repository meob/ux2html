...
+ Linux/GNU  (many distributions tested: RedHat, CentOS, RockyLinux, OEL, Fedora, Ubuntu, Debian, Alpine, Photon OS, SUSE, ...)
+ SUN/Oracle SunOS/Solaris  
+ IBM AIX  
+ HP HP-UX, Tru64  
+ Apple Darwin/MAC OS X  
+ Microsoft MS-Windows (yes! Windows too... using Cygwin/Win)  
...
if You find a bug for a version specific version we will try to fix it!  
The script is easly extendible with plug-in and custom configuration.


## Installation
> mkdir /usr/local/amm  
> cd /usr/local/amm  
> wget --no-check-certificate https://www.meo.bogliolo.name/pub/ux2html.zip    # Or Github
> unzip ux2html.zip
...
## Plug-ins
Plug-in directory contains several additional custom plug-ins.
To use a specific plug-in move it in /usr/local/amm
> cd /usr/local/amm  
> mv Plug-in/ux2p.020.Sap.sh ..

## Crontab configuration
ux2html statistics can be easly scheduled by crontab with quotes `:  
